...
class GoogleGeocode {
  ...
  getRestaurant(latitude,longitude) {
    let geoPlaceURL = `https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${latitude},${longitude}&radius=300&type=restaurant&keyword=cruise&key=${process.env.PLACEKEY}`
  ...
  getToDo(latitude, longitude) {
    let geoDoURL = `https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${latitude},${longitude}&radius=300&type=beauty_salon|book_store|cafe|casino|night_club|jewelry_store|pet_store|spa|stadium|storage|store|subway_station|travel_agency&key=${process.env.PLACEKEY}`
